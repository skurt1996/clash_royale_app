...
# Dictionary is needed to use the table_exists method
...
def create_connection(host="localhost", port=3306, user=os.environ.get("MYSQL_USERNAME"), password=os.environ.get("MYSQL_PASSWORD"), database="clash_royale"):
    ...
    Args:
        ...
        port (int): The port number to connect to the MySQL server.
    ...
    Returns:
        cnx: A connection object representing the connection to the clash_royale database.
            The connection object can be used to execute database operations.

    Raises:
        mysql.connector.Error: If an error occurs while establishing the connection.
    """
    cnx = None
    try:
...
def insert_with_error_handling(cnx, query, params, recursive_insertion=False):
    ...
    Args:
        cnx (mysql.connector.MySQLConnection): The MySQL connection object.
        query (str): The SQL insert query.
        params (list or tuple): The parameter values to be inserted into the query.
        recursive_insertion (bool, optional): Specifies whether recursive insertion should be attempted
            in case of a duplicate entry error. Defaults to False.

    Returns:
        bool: Returns True if the query was successfully executed, False otherwise.
...
def get_player_info(player_tag):
    ...
    Returns:
        Dictionary of column informations or None if player not found.
    """
    with create_connection() as cnx:
...
def extract_battles(battles):
    ...
    Args:
        battles (List of tuples): Tuples contain battle data.

    Returns:
        list: A list of dictionaries of battle data.
    """
    player_ids = set()
...
def get_query_and_params(latest_battle_time, game_mode, player_tag, enemy_tag, limit):
    ...
    Parameters:
        latest_battle_time (str): The timestamp for filtering battles.
        game_mode (str): The game mode for filtering battles.
        player_tag (str): The player"s tag for filtering battles.
        enemy_tag (str): The enemy player"s tag for filtering battles.
        limit (bool): A flag indicating whether to limit the number of results.

    Returns:
        tuple: A tuple containing the SQL query and its parameters for database querying.
    """
    query_params_dict = {
        # All battles
...
def get_battles(latest_battle_time="2200-12-31", game_mode="ALL", player_tag=None, enemy_tag=None, limit=True):
    ...
    Args:
        player_tag (str): The tag of the player to retrieve battles for.
        latest_battle_time (str): Needed to query the battles before this battle_time (see SQL statement)
        game_mode (str): The game mode to filter battles by. Default is "ALL".
        enemy_tag (str): The tag of the enemy player. Default is None. That means all enemies are considered.
        limit (bool): If True, only 10 elements are retrieved.
    
    Returns:
        list: A list of dictionaries of battle data.

    Note:
        Returns empty list if player_tag not in the database or if no battles are given for the player_tag.

    Raises:
        TypeError: If there"s an issue with the input types or data.
        mysql.connector.Error: If there"s an error during the database query.
    """
    with create_connection() as cnx:
        try:
            with cnx.cursor() as cursor:
...
def parse_princess_tower_hp(princess_tower_hp):
    ...
    Args:
        princess_tower_hp (str): Princess tower HP data as a string representation of a list.

    Returns:
        tuple: A tuple containing the princess tower HP values. If the input is None, returns (0, 0).
               If there is only one tower HP value, returns (tower_hp, 0). Otherwise, returns (tower_hp1, tower_hp2).
    """
...
def insert_battle(cursor, battle_info):
    ...
    Args:
        cursor: MySQLCursor
        battle_info: tuple
            Information about the battle, including battle time, type, and game mode.
...
def insert_score(cursor, player_data, player_deck, battle_id):
    ...
    Args:
        cursor: MySQLCursor
        crowns: int
            Number of crowns achieved in the battle.
        king_tower_hp: int
            Hit points of the player"s King Tower.
        princess_tower_1_hp: int
            Hit points of the player"s first Princess Tower.
        princess_tower_2_hp: int
            Hit points of the player"s second Princess Tower.
        elixir_leaked: int
            Amount of elixir leaked by the player.
        deck_string: str
            String representation of the player"s deck.
        battle_id: int
            ID of the battle the score belongs to.
        player_id: int
            ID of the player.
...
def check_for_battle_duplicate(time, player1_id, player2_id):
    ...
    Parameters:
        time (str): The time in the format "%Y-%m-%d %H:%M:%S".
        player1_id (int): The ID of the first player.
        player2_id (int): The ID of the second player.

    Returns:
        bool: True if a duplicate battle with the same time and players" IDs exists,
              False otherwise.
    """
...
def insert_new_battles():
    ...
    Raises:
        mysql.connector.Error: If an error occurs during database operations.
    """
